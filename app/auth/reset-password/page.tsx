...
import { useForgotCheck, useForgotSubmit } from '~/query/auth'
...
const Page = () => {
  ...
  const { mutate: check } = useForgotCheck({
    onSuccess: () => {
      setDisable(false)
    },
    onError: () => {
      router.push('/')
      toast.error('The link has expired. Please request a new reset link.')
    },
  })
  const { mutate: confirm, isPending } = useForgotSubmit({
    onSuccess: () => {
      router.push('/')
